...
class FlovaWs {
 public:
  ...
  FlovaWs(FlovaLinkStream& transport, FlovaEntropySource& entropy)
  ...
  bool sendBinaryCoalesced(const uint8_t* data, size_t length, uint8_t* workspace,
                           size_t capacity) {
    size_t wireLength = 0;
  ...
  bool prepareBinary(const uint8_t* data, size_t length, uint8_t* workspace,
  ...
  int read(uint8_t* destination, size_t capacity) {
    ...
    if (!serviceControl()) return failRead(Error::Transport);
    if (pendingControlLength_) return 0;
    ...
    for (;;) {
      ...
      if (frameControl_) {
        ...
        if (pendingControlLength_) return static_cast<int>(written);
        continue;
      }
  ...
  bool controlPending() const { return pendingControlLength_ != 0; }
  bool serviceControl() {
    if (!pendingControlLength_ || transport_.writeBusy()) return true;
    const size_t written = transport_.write(pendingControl_, pendingControlLength_);
    if (!written || written > pendingControlLength_) return false;
    pendingControlLength_ -= written;
    if (pendingControlLength_)
      memmove(pendingControl_, pendingControl_ + written, pendingControlLength_);
    return true;
  }
  ...
  void abort() { state_ = State::Closed; pendingControlLength_ = 0; }
 ...
 private:
  ...
  bool sendFrame(uint8_t opcode, const uint8_t* payload, size_t length) {
    if (state_ != State::Open || opcode < 0x8 || opcode > 0xA ||
        length > kMaximumControlBytes || pendingControlLength_) return false;
    pendingControl_[0] = static_cast<uint8_t>(0x80 | opcode);
    pendingControl_[1] = static_cast<uint8_t>(0x80 | length);
    for (size_t i = 0; i < 4; ++i) pendingControl_[2 + i] = randomByte();
    for (size_t i = 0; i < length; ++i)
      pendingControl_[6 + i] = payload[i] ^ pendingControl_[2 + (i & 3)];
    pendingControlLength_ = length + 6;
  ...
  uint8_t pendingControl_[kMaximumControlBytes + 6] = {};
  size_t pendingControlLength_ = 0;
